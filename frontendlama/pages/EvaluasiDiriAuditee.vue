                    ...
                    <table class="table table-sm mb-0">
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            <span v-if="evaluasiDiri && evaluasiDiri.dokumen" class="badge bg-success">Ada</span>
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        const [hirarkiRes, evaluasiRes] = await Promise.all([
          ...
          fetch(`/api/edi.php?action=get_evaluasi&on_id=${this.currentOnId}`),
        ]);
    ...
    async openModal(subssm, ssm, psm, standar) {
      ...
      try {
        ...
        const ediRes = await fetch(`/api/edesk.php?action=get_evaluasi&on_id=${onData.on_id}`);
    ...
    async handleSubmit() {
      ...
      fd.append('auditor_id', 1); // Ganti dengan id auditor login
      fd.append('nm_id', this.formData.nilai);
